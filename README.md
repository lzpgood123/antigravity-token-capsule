...
* 💰 **财务计费估算与双计费联动 (Gemini 3.8 Flash 基准定价)**  
  * **权威基准模型**：折算费用严格基于 **Google Gemini 3.8 Flash** 官方标准计费：
    | 计量项目 | 费率标准 | 说明 |
    | :--- | :--- | :--- |
...
* ⚡ **上下文自动压缩遥测 (Context Auto-Compaction Telemetry)**  
  ...
  * **Hero 预警徽章联动**：当发生压缩时（`>= 1 次`），核心百分比旁自动亮起橙色警示徽章（如 `⚡ 压缩 2 次`），并在下方 Summary 列表中提供 `上下文自动压缩` 状态行，让开发者一目了然得知模型精确记忆衰减情况。
* 🎨 **5 套主题风格与桌面极简交互**  
