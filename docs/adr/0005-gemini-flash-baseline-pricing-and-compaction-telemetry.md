...
1. **计费模型透明度**：此前数据引擎直接按每百万 Token 费率（输入 $0.75、输出 $3.75、缓存 $0.15）计算会话美元花费，但客户端 UI 与文档均未显式声明基准模型，导致开发者无法确认折算依据，容易误判为 Claude 3.5 Sonnet 等高价模型或与官方账单发生认知脱节。
...
1. **确立 Gemini 3.8 Flash 为全局基准计费模型 (Baseline Pricing Model)**：
   * **标准化费率**：输入 \$0.75/1M tokens、输出 \$3.75/1M tokens、Prompt 缓存命中 \$0.15/1M tokens。
